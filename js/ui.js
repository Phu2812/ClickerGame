    ...
    for (const type in containers) {
        if(containers[type]) {
            ...
            if (type === 'click') {
                ...
                GAME_DATA.upgrades.click.filter(u => u.category === 'pure').forEach(upgrade => renderUpgradeCardHTML(upgrade, containers[type]));
                ...
                GAME_DATA.upgrades.click.filter(u => u.category === 'elemental').forEach(upgrade => renderUpgradeCardHTML(upgrade, containers[type]));
            } else {
                GAME_DATA.upgrades[type].forEach(upgrade => renderUpgradeCardHTML(upgrade, containers[type]));
            }
        }
    }
}

function renderUpgradeCardHTML(upgrade, container) {
...
function handleUpgradeClick(id) {
    ...
    if (upgradeData.type === 'skill' && currentLevel > 0) {
        openEquipModal(id);
    } else {
        buyUpgrade(id);
    }
...
function updateUpgradesUI() {
    ['click', 'dps', 'economy', 'skill'].forEach(type => {
        GAME_DATA.upgrades[type].forEach(upgrade => {
            ...
            const levelDisplayEl = document.getElementById(`level-display-${upgrade.id}`);
            const extraInfoEl = document.getElementById(`extra-info-${upgrade.id}`);
            const nextLevelInfoEl = document.getElementById(`next-level-info-${upgrade.id}`);
            const costTextEl = document.getElementById(`cost-text-${upgrade.id}`);
            const btnBuyEl = document.getElementById(`btn-buy-${upgrade.id}`);
            ...
            if (levelDisplayEl && levelDisplayEl.innerHTML !== levelDisplay) levelDisplayEl.innerHTML = levelDisplay;
            if (extraInfoEl && extraInfoEl.innerHTML !== extraInfo) extraInfoEl.innerHTML = extraInfo;
            if (nextLevelInfoEl && nextLevelInfoEl.innerHTML !== nextLevelInfo) nextLevelInfoEl.innerHTML = nextLevelInfo;

            if (type === 'skill') {
                const canBuyOrEquip = (currentLevel > 0) || (currentLevel === 0 && canAfford);
                let buttonText = currentLevel > 0 ? 'Trang bị' : `<i class="fa-solid fa-coins text-yellow-400"></i> ${currentCost.toLocaleString()}`;
                
                if (btnBuyEl.innerHTML !== buttonText) btnBuyEl.innerHTML = buttonText;
                btnBuyEl.disabled = (!canAfford && currentLevel === 0);
            } else {
                let costText = `<i class="fa-solid fa-coins text-yellow-500 mr-1"></i> ${currentCost.toLocaleString()}`;
                let buttonText = currentLevel === 0 ? 'Mở khóa' : 'Nâng cấp';
                if (isMaxLevel) {
                    costText = 'Đã tối đa';
                    buttonText = 'Tối đa';
                    if (nextLevelInfoEl) nextLevelInfoEl.innerHTML = ''; 
                }

                if (costTextEl && costTextEl.innerHTML !== costText) costTextEl.innerHTML = costText;
                if (btnBuyEl.innerHTML !== buttonText) btnBuyEl.innerHTML = buttonText;
                btnBuyEl.disabled = !canAfford;
            }
        });
    });
}

function initGemUpgrades() {
...
function updateGemUpgradesUI() {
    GAME_DATA.gemUpgrades.forEach(upgrade => {
        ...
        const infoEl = document.getElementById(`gem-info-${upgrade.id}`);
        const nextEl = document.getElementById(`gem-next-${upgrade.id}`);
        const btnBuyEl = document.getElementById(`btn-gem-buy-${upgrade.id}`);

        if(!btnBuyEl) return;

        const infoHTML = `Cấp <span class="text-white font-bold">${currentLevel}</span> | Bonus: <span class="text-cyan-400">+${currentBonus}%</span>`;
        const nextHTML = `Tiếp theo: +${nextBonus}%`;
        const btnHTML = `<span>Nâng Cấp</span><span class="text-xs text-cyan-200 drop-shadow-md"><i class="fa-solid fa-gem"></i> ${currentCost}</span>`;

        if (infoEl.innerHTML !== infoHTML) infoEl.innerHTML = infoHTML;
        if (nextEl.innerHTML !== nextHTML) nextEl.innerHTML = nextHTML;
        if (btnBuyEl.innerHTML !== btnHTML) btnBuyEl.innerHTML = btnHTML;
        btnBuyEl.disabled = !canAfford;
    });
}
